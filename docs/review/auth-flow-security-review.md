...
一方、connpass APIを呼び出す次の検索APIにはログイン確認がなかった。
...
未ログイン状態で実際にアクセスしたところ、両方ともHTTP 200が返り、検索結果を取得できた。

APIキーそのものの露出やユーザーの非公開データ漏洩は確認されていないが、第三者がConnLogのサーバーとconnpass API keyを使って検索を実行できるため、テストユーザー招待前の修正対象とする。
...
両方とも、ログインCookieを送らないcurlから検索結果を取得できた。
...
connpassのイベント情報自体は公開情報のため、今回の結果だけでユーザーの非公開データ漏洩とは判断しない。

また、`CONNPASS_API_KEY`の値そのものが漏洩した形跡も確認されていない。
...
ログイン済みのユーザーは、従来どおり検索できるようにする。

現在は古いPages Router形式で実装されているため、ほかの認証済みAPIと同じApp Router形式への移行を有力候補とする。

修正は、この監査用ブランチでは行わず、別の`fix/`ブランチで実施する。
...
- GET以外のHTTPメソッドを拒否する
- `event_id`を想定した形式へ制限する
- `nickname`をサーバー側でも安全にURLエンコードする
- `start`を0以上の整数へ制限する
- `count`へ上限を設ける
- connpass APIのエラーを利用者へ必要以上に露出しない
...
2026年7月22日に、Supabase Dashboardで次を変更した。
...
この監査資料をマージした後、次を別タスク・別PRで対応する。

### 優先対応

- `/api/search-event`へログイン確認を追加
- `/api/search-user`へログイン確認を追加
- 未ログイン時に401を返すことを実地確認
- ログイン済みの通常検索が成功することを確認
